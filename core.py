# -*- coding: utf-8 -*-
"""核心：配置 / 预设 / 角色字典 / LoRA 管理 / 提示词优化 / 跑图队列"""
...
DEFAULT_CONFIG = {
    "onebot": {"ws_host": "127.0.0.1", "ws_port": 8080, "access_token": ""},
    "webui": {"base_url": "http://127.0.0.1:7860", "timeout": 600},
    "llm": {"base_url": "https://api.x.ai/v1", "api_key": "", "model": "grok-4", "proxy": ""},
    "gen": {
...
CFG = load_json(CONFIG_PATH) or DEFAULT_CONFIG
for k, v in DEFAULT_CONFIG.items():  # 补全新增字段
    if k not in CFG:
        CFG[k] = v
...
def scan_loras(lora_dir):
    ...
    # 覆盖里有但扫描不到的（目录变了）也保留显示
    for stem, ov in overrides.items():
        if stem not in found:
            found[stem] = {
                "stem": stem, "file_name": "", "model_name": ov.get("model_name", "（未扫描到）"),
...
def save_optimizer_prompt(text):
...
async def run_job(job):
    ...
    parts = [gen.get("quality_prefix", "").strip().rstrip(","),
    ...
    w, h = job.get("size") or (int(gen.get("width", 1024)), int(gen.get("height", 1024)))
    ...
    rec = {
        "time": time.strftime("%m-%d %H:%M:%S"), "file": fname, "prompt": prompt,
        "negative": negative, "style": job.get("style"), "size": [w, h],
        "source": job.get("source", "qq"), "user": job.get("user_name", ""),
        "loras": job["loras"], "duration": duration,
    }
    HISTORY.append(rec)
    history_save()
